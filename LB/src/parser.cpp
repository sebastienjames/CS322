...
namespace LB {
  ...
  // struct Instruction_function_declaration:
  //   pegtl::seq<
  ...
  //   struct Instructions_rule_scope: 
  //     pegtl::star<
  //       Instruction_rule_scope
  //     > {};
  ...
  // struct line:
  //   pegtl::sor<
  //     pegtl::seq< pegtl::at<Instruction_function_declaration>     , Instruction_function_declaration     >,
  //     Instruction_rule
  //   > {};
  ...
  template<> struct action < names > {
    template< typename Input >
      ...
      auto n = std::count(arg_string.begin(), arg_string.end(), ',');
      print("N is " + to_string(n));
    
      for (int i = 0; i <= n; i++) {
        auto item = parsed_items.back();
        print("Item is " + item->to_string());
        names.insert(names.begin(), parsed_items.back());
        print("inserted");
        parsed_items.pop_back();
        print("popped");
      }
  ...
  template<> struct action < Instruction_goto_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Adding a goto instruction");
  ...
  template<> struct action < Instruction_continue_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Adding a continue instruction");
  ...
  template<> struct action < Instruction_while_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Adding a while assignment instruction");
      ...
      auto i = new Instruction_while(t1, op, t2, true_label, false_label);
      i->lineNumber = in.position().line;
  ...
  template<> struct action < Instruction_if_rule > {
    template< typename Input >
      ...
      auto i = new Instruction_if(t1, op, t2, true_label, false_label);
      i->lineNumber = in.position().line;
  ...
  template<> struct action < Instruction_start_scope_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Adding a start scope instruction");
  ...
  template<> struct action < Instruction_end_scope_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Adding a end scope instruction");
