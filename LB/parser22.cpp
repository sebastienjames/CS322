...
namespace LB {
  ...
  struct Instruction_start_scope_rule:
    pegtl::one<'{'> {};
    
  struct Instruction_end_scope_rule:
    pegtl::one<'}'> {};
  ...
  struct Instruction_rule:
    pegtl::sor<
      pegtl::seq< pegtl::at<Instruction_start_scope_rule>     , Instruction_start_scope_rule      >,
      pegtl::seq< pegtl::at<Instruction_end_scope_rule>     , Instruction_end_scope_rule      >,
  ...
  struct Scope_rule:
    pegtl::seq<
      Instruction_start_scope_rule,
      seps,
      pegtl::star<
        Instruction_rule,
        seps
      >,
      Instruction_end_scope_rule
    > {};
 ...
 struct Function_rule:
  ...
  template<> struct action < Instruction_start_scope_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Starting a new scope");
      auto currentF = p.functions.back();
      // current_scope += 1;
      // vector<Instruction*> new_scope;
      // parsed_instructions.push_back(new_scope);

      // for (auto x : parsed_instructions) {
      //   print("new level");
      //   for (auto y : x) {
      //     print("Item");
      //     y->printInstruction();
      ...
      // print("new scope level" + to_string(current_scope));
      auto i = new Instruction_start_scope();
  ...
  template<> struct action < Instruction_end_scope_rule > {
    template< typename Input >
	  static void apply( const Input & in, Program & p){
      print("Starting a end scope");
      auto currentF = p.functions.back();
      auto i = new Instruction_end_scope();
  ...
  // template<> struct action < names > {
  //   template< typename Input >
	//   static void apply( const Input & in, Program & p){
  //     print("Adding a names item");
  //     auto names_string = in.string();
  ...
  //     auto n = std::count(names_string.begin(), names_string.end(), ',');
  //     for (int i = 0; i <= n; i++) {
  //       names.push_back(parsed_items.back());
  //       parsed_items.pop_back();
  //     }

      

  //     auto namesItem = new Names(names);
  //     print("here");
  //     parsed_items.push_back(namesItem);
  //     print("there");
  //   }
  // };
  ...
  template<> struct action < Instruction_declaration_rule > {
    template< typename Input >
      ...
      auto names = parsed_items.back();
      parsed_items.pop_back();
      cout << names->to_string();
      auto type = parsed_items.back();
      parsed_items.pop_back();

      
      
      auto i = new Instruction_declaration(type, names);
      i->lineNumber = in.position().line;
  ...
  template<> struct action < Instruction_if_rule > {
    template< typename Input >
      ...
      auto false_label = parsed_items.back();
      parsed_items.pop_back();

      auto true_label = parsed_items.back();
      parsed_items.pop_back();
      ...
      auto i = new Instruction_if(t1, cmp, t2, true_label, false_label);
      i->lineNumber = in.position().line;
      currentF->instructions.push_back(i);
    }
  };
  ...
  template<> struct action < Instruction_while_rule > {
    template< typename Input >
      ...
      auto i = new Instruction_while(t1, cmp, t2, true_label, false_label);
      i->lineNumber = in.position().line;
      currentF->instructions.push_back(i);
    }
  };
  ...
  // template<> struct action < Scope_rule > {
  //   template< typename Input >
	//   static void apply( const Input & in, Program & p){
  //     print("added a scope");
  ...
  //     // int64_t scope_size = parsed_instructions[current_scope].size();
  //     // print(in.string());
  //     // for (int64_t i = 0; i < scope_size; i++) {
  //     //   instructions.insert(instructions.begin(), parsed_instructions[current_scope].back());
  //     //   parsed_instructions.pop_back();
  //     // }
  //     // auto i = new Scope(instructions);
  //     // current_scope -= 1;
  //     // i->lineNumber = in.position().line;
  //     // currentF->instructions.push_back(i);
  //   }
